  ...
  return (
    <div className="space-y-4">
      {MEAL_ORDER.map((meal) => {
        ...
        const subtotal = Math.round(items.reduce((s, m) => s + m.calories, 0))
        const protein = Math.round(items.reduce((s, m) => s + m.protein, 0))
        return (
          <section key={meal} className="overflow-hidden rounded-3xl border border-border bg-card">
            <header className="flex items-baseline justify-between bg-muted/50 px-5 py-3">
              <h3 className="text-base font-bold text-foreground">{MEAL_LABELS[meal]}</h3>
            ...
            <ul className="divide-y divide-border">
              {items.map((m) => (
                <MealRow key={m.id} entry={m} onRemove={() => onRemove(m.id)} />
              ))}
            </ul>
          </section>
        )
...
function MealRow({ entry, onRemove }: { entry: MealEntry; onRemove: () => void }) {
  return (
    <li className="flex items-center gap-3 px-5 py-3">
      ...
      <div className="min-w-0 flex-1">
        <div className="flex items-center gap-1.5">
          ...
          {entry.photo ? <Camera className="size-3.5 shrink-0 text-muted-foreground" aria-label="含照片" /> : null}
        </div>
        <p className="text-sm text-muted-foreground">
          {entry.portion ? `${entry.portion}・` : ""}
          {Math.round(entry.calories)} kcal・蛋白 {Math.round(entry.protein)}g・碳 {Math.round(entry.carbs)}g・脂{" "}
